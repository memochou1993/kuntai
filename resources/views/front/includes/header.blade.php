<div class="container">
    <div class="d-flex flex-wrap box" id="header">
        <div class="col-xl-4 col-lg-6 col-md-6 col-sm-8">
            <div>
                <a href="{{ route('front.home.index') }}"><img src="{{ asset('public/images/logo.png')}}" alt="KUNTAI" class="box-logo"/></a>
            </div>
        </div>
                        ...
                        <div class="container">
                            ...
                            <div class="collapse navbar-collapse" id="nav-genre">
                                <ul class="navbar-nav">
                                    ...
                                    <li class="nav-item dropdown
                                            @foreach (explode(',', trim($request->nq)) as $narrowed_query)
                                                    @if (strpos($narrowed_query, 'region') !== false)
                                                        {{ 'active' }}
                                                    @endif
                                            @endforeach">
                                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                            @foreach ($item_catalogs as $item_catalog)
                                                @if ($item_catalog->type == 'Region')
                                                    <a href="{{ route('front.items.filter', ['nq' => 'region:'.$item_catalog->name]) }}" class="dropdown-item
                                                            @foreach (explode(',', trim($request->nq)) as $narrowed_query)
                                                                    @if (strpos($narrowed_query, $item_catalog->name) !== false)
                                                                        {{ 'active' }}
                                                                    @endif
                                    ...
                                    <li class="nav-item dropdown
                                            @foreach (explode(',', trim($request->nq)) as $narrowed_query)
                                                    @if (strpos($narrowed_query, 'maker') !== false)
                                                        {{ 'active' }}
                                                    @endif
                                            @endforeach
                                        ">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                            @foreach ($item_catalogs as $item_catalog)
                                                @if ($item_catalog->type == 'Maker')
                                                    <a href="{{ route('front.items.filter', ['nq' => 'maker:'.$item_catalog->name]) }}" class="dropdown-item
                                                            @foreach (explode(',', trim($request->nq)) as $narrowed_query)
                                                                    @if (strpos($narrowed_query, $item_catalog->name) !== false)
                                                                        {{ 'active' }}
                                                                    @endif
                                    ...
                                    <li class="nav-item dropdown
                                            @foreach (explode(',', trim($request->nq)) as $narrowed_query)
                                                    @if (strpos($narrowed_query, 'brand') !== false)
                                                        {{ 'active' }}
                                                    @endif
                                            @endforeach
                                        ">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                            @foreach ($item_catalogs as $item_catalog)
                                                @if ($item_catalog->type == 'Brand')
                                                    <a href="{{ route('front.items.filter', ['nq' => 'brand:'.$item_catalog->name]) }}" class="dropdown-item
                                                            @foreach (explode(',', trim($request->nq)) as $narrowed_query)
                                                                    @if (strpos($narrowed_query, $item_catalog->name) !== false)
                                                                        {{ 'active' }}
                                                                    @endif
